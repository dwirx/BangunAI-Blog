...
| **Mermaid Diagrams** | Flowchart, sequence diagram, pie chart — auto-render dengan tema terang/gelap, unique ID per diagram |
...
| [Mermaid 11](https://mermaid.js.org) | Diagram rendering (flowchart, sequence, pie, dll) |
...
│   ├── MermaidDiagram.tsx     # Mermaid chart renderer (unique ID, theme-aware)
...
│
├── pages/
│   ├── Index.tsx              # Homepage (featured + latest posts)
...
|------|---------|-----------|
| `/` | Homepage | Featured posts & tulisan terbaru |
...
**Mendukung:** `graph`, `sequenceDiagram`, `pie`, `classDiagram`, `stateDiagram`, `erDiagram`, `gantt`, `flowchart`, dan lainnya.
...
- **Mermaid render** menggunakan `mermaid.render()` dengan unique ID per diagram
...
### Vite (`vite.config.ts`)
- MDX plugin dengan remark-gfm, remark-math, remark-frontmatter
- Rehype plugins: rehype-katex, rehype-pretty-code (Shiki)
- Path alias `@/` → `src/`
