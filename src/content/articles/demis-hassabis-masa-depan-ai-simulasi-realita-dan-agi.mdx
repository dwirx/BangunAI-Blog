...
```mermaid
graph TD
    A["🧠 Foundation Model\n(LLM)"] --> B["Mengusulkan Kandidat Solusi\n(seed programs)"]
    B --> C["Evolutionary Engine\n(mutasi + crossover + seleksi)"]
    C --> D{"Solusi Lebih Baik\ndari yang Diketahui?"}
    D -->|Ya| E["✅ Breakthrough!\n(contoh: faster matrix multiplication)"]
    D -->|Tidak| F["Terus Iterasi\n(generasi berikutnya)"]
    F --> C
    E --> G["Masuk ke Base Knowledge\nuntuk iterasi selanjutnya"]
```
...
```mermaid
graph TD
    A["❓ Apakah X Sadar?"] --> B{Perilaku seperti\nmakhluk sadar?}
    B -->|Tidak| C["❌ Kemungkinan Besar Tidak Sadar"]
    B -->|Ya| D{Substrat sama\n(berbasis karbon)?}
    D -->|Ya| E["✅ Sangat Mungkin Sadar\n(parsimoni: asumsi terbaik)"]
    D -->|Tidak\n(silicon, dll)| F["🤷 Tidak Bisa Dipastikan\n(frontier filosofis)"]
```
